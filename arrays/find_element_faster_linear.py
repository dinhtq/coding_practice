...
arr = [13, 18, 25, 2, 8, 10]
num = 8
print(shifted_array_search(arr, num) == 4)
...
We can obviously do this problem in linear time if we looked at each element in the array. However, we need to do it faster than linear time.
 A big clue should be that the array of integers was previously sorted, and then rotated. If it was just sorted, we could do a binary search. However, this array was also rotated, so we can't do a regular binary search. We can modify it slightly to get to where we want it, however.
