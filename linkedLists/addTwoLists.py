...
# tests
desc = 'short same lengths lists'
head1 = Node(1)
appendToList(head1, 2)
head2 = Node(3)
appendToList(head2, 4)
resultNode = add(head1, head2)
assertEqual(resultNode.val, 4, 'test')
...
We'll start at the head of the two nodes, and compute sum % 10. We write this down,
move the two nodes up and add a carry if the sum was greater than 10.
A tricky part here is to finding the terminating condition.
We can see that this happens when there is no more carry and the two linked lists have reached the end.
Since this is tricky to represent in code, we instead extend the nodes one more until they are both None and carry is 0 and then simply return None.
